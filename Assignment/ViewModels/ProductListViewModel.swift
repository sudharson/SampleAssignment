...
class ProductListViewModel: NSObject {
    ...
    var unGroupedProductsList = [ProductModel]()
    var groupedProductsList : [(key:String,values:[ProductModel])]?
    ...
    func fetchList(searchKeyword: String, completionHandler: @escaping (_ error: Error?, _ isChanged: Bool)-> ()) {
        ...
        if productModel.count > 0 {
            if (self.isGrouped ?? false) {
                ...
                self.groupedProductsList = sortedDict.compactMap({(key:$0,values:$1)})
            } else {
                self.unGroupedProductsList.removeAll()
                self.unGroupedProductsList = productModel
            }
                        ...
                        Product.parseAndSaveResponse(response: productResponseDict) { isSuccess in
                            if isSuccess {
                                ...
                                if (self?.isGrouped ?? false) {
                                    self?.dataSourceDict = Dictionary.init(grouping: productModel, by: { $0?.category ?? "" })
                                    let sortedDict = self?.dataSourceDict.sorted(by: { $0.key < $1.key})
                                    self?.groupedProductsList = sortedDict?.compactMap({(key:$0,values:$1.sorted(by: {$0.id ?? 0 < $1.id ?? 0}))})
                                } else {
                                    self?.unGroupedProductsList.removeAll()
                                    self?.unGroupedProductsList = productModel
                                }
    ...
    func fetchQueriedList(searchKeyword: String, completionHandler: @escaping (_ error: Error?, _ isChanged: Bool)-> ()){
        ...
        if (self.isGrouped ?? false) == true {
            ...
            self.unGroupedProductsList.removeAll()
            self.unGroupedProductsList = productModel.sorted(by: {$0.id ?? 0 < $1.id ?? 0})
        }
        completionHandler(nil,true)
    }
    ...
    func sortByAscendingOrder() {
        if (self.isGrouped ?? false) == true {
            let sortedDict = self.dataSourceDict.sorted(by: { $0.key < $1.key})
            self.groupedProductsList = sortedDict.compactMap({(key:$0,values:$1.sorted(by: {$0.price ?? 0 < $1.price ?? 0}))})
        } else {
            self.unGroupedProductsList.sort(by: {$0.price ?? 0 < $1.price ?? 0})
        }
    }
    ...
    func sortByNameAscending() {
        if (self.isGrouped ?? false) == true {
            let sortedDict = self.dataSourceDict.sorted(by: { $0.key < $1.key})
            self.groupedProductsList = sortedDict.compactMap({(key:$0,values:$1.sorted(by: {$0.title ?? "" < $1.title ?? ""}))})
        } else {
            self.unGroupedProductsList.sort(by: {$0.title ?? "" < $1.title ?? ""})
        }
    }
    ...
    func sortByRatingAscending() {
        if (self.isGrouped ?? false) == true {
            let sortedDict = self.dataSourceDict.sorted(by: { $0.key < $1.key})
            self.groupedProductsList = sortedDict.compactMap({(key:$0,values:$1.sorted(by: {$0.rate ?? 0 > $1.rate ?? 0}))})
        } else {
            self.unGroupedProductsList.sort(by: {$0.rate ?? 0 > $1.rate ?? 0})
        }
    }
}
